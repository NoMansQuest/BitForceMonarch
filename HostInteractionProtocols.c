...
PROTOCOL_RESULT Protocol_info_request(void)
{
	...
	*/
	...
	// Perform chip-by-chip test
		...
		for (char iScanChip = 0; iScanChip < TOTAL_SUBCHIPS_PER_BUS; iScanChip++)
		{
			...
			for (unsigned char tt = 0; tt < TOTAL_ENGINES_PER_SUBCHIP; tt++)
			{
	...
	// Report all busy engines
		...
		for (char nmc = 0; nmc < TOTAL_SUBCHIPS_PER_BUS; nmc++)
		{
			for (char nme = 0; nme < TOTAL_ENGINES_PER_SUBCHIP; nme++)
			{
	...
	// Proceed
		...
		// Now run chip-by-chip diagnostics
			...
			unsigned char umx = 0;
			for (umx = 0; umx < TOTAL_SUBCHIPS_PER_BUS; umx++)
			{
				if (!CHIP_EXISTS(umx)) continue;
				...
				// Are we running diagnostics on the engines too?
					...
					for (i_engine = 0; i_engine < TOTAL_ENGINES_PER_SUBCHIP; i_engine++)
					{
						// Reset Watchdog
						WATCHDOG_RESET;						
						bFailedEngine = FALSE;
...
PROTOCOL_RESULT  Protocol_PIPE_BUF_FLUSH_EX(void)
{
	...
	for (unsigned char iChipIndex = 0; iChipIndex < TOTAL_SUBCHIPS_PER_BUS; iChipIndex++)
	{
		// Add it to stream
