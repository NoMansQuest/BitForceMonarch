...
/*************** Product Model *********************/
#define __PRODUCT_MODEL_SINGLE_CHIP__
#define __PRODUCT_MODEL_DOUBLE_CHIP__

/********* TOTAL CHIPS INSTALLED ON BOARD **********/
#define TOTAL_ENGINES_PER_SUBCHIP 16

#if defined(__PRODUCT_MODEL_SINGLE_CHIP__) // Each single 28nm chip will contain 8 sub-chips with each 128 engines
	#define	 TOTAL_SUBCHIPS_PER_BUS	8
#else
	#define	 TOTAL_SUBCHIPS_PER_BUS	16 
#endif
...
//#define __SHOW_PIPE_TO_USB_LOG			
	...
	volatile unsigned int __ENGINE_LOWRANGE_SPREADS[TOTAL_SUBCHIPS_PER_BUS][16];
	volatile unsigned int __ENGINE_HIGHRANGE_SPREADS[TOTAL_SUBCHIPS_PER_BUS][16];
#endif
...
#define __CHIP_FREQUENCY_DETECT_AND_REPORT 1     // Round-trip
...
/*************** UNIT ID STRING ********************/
#define  UNIT_ID_STRING			"BitForce MONARCH 1.0\n"

// Unit Identification String
#define UNIT_FIRMWARE_ID_STRING	"ID: BitForce MONARCH Version 1.0"
...
volatile unsigned short GLOBAL_ChipActivityLEDCounter[TOTAL_SUBCHIPS_PER_BUS];
...
// Other variables
unsigned char COMM_USING_PCIE;
unsigned char COMM_USING_USB1;
unsigned char COMM_USING_USB2;

int global_vals[6];

// Contains information about frequency of all the chips installed
extern unsigned int  GLOBAL_CHIP_FREQUENCY_INFO[TOTAL_SUBCHIPS_PER_BUS];
extern unsigned char GLOBAL_CHIP_PROCESSOR_COUNT[TOTAL_SUBCHIPS_PER_BUS];
unsigned int GLOBAL_CHIP_PROCESSOR_ENGINE_LOWBOUND[TOTAL_SUBCHIPS_PER_BUS][16]; // Calculated on initialization
unsigned int GLOBAL_CHIP_PROCESSOR_ENGINE_HIGHBOUND[TOTAL_SUBCHIPS_PER_BUS][16]; // Calculated on initialization

// Monitoring Authority arrays
unsigned char GLOBAL_ENGINE_PROCESSING_STATUS[TOTAL_SUBCHIPS_PER_BUS][16]; // If 1, the engine is processing
unsigned char GLOBAL_ENGINE_PROCESSING_FAILURE_SCORES[TOTAL_SUBCHIPS_PER_BUS][16]; // If this number reaches 5 failures, we'll decommission the engine
unsigned int  GLOBAL_ENGINE_PROCESSING_START_TIMESTAMP[TOTAL_SUBCHIPS_PER_BUS][16]; // When did this engine start processing?

// Maximum operating time for engines
unsigned int  GLOBAL_ENGINE_MAXIMUM_OPERATING_TIME[TOTAL_SUBCHIPS_PER_BUS][16];
#if defined(QUEUE_OPERATE_ONE_JOB_PER_CHIP)
...
unsigned int __chip_existence_map[TOTAL_SUBCHIPS_PER_BUS]; // Bit 0 to Bit 16 in each word says if the engine is OK or not...
...
#if defined(QUEUE_OPERATE_ONE_JOB_PER_CHIP)
	volatile unsigned int GLOBAL_LastJobIssuedToChip[TOTAL_SUBCHIPS_PER_BUS];
#endif
