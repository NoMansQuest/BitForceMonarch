...
int	 ASIC_get_job_status(unsigned int *iNonceList, 
	 					 unsigned int *iNonceCount, 
	 					 const char iCheckOnlyOneChip, 
	 					 const char iChipToCheck);
...
void ASIC_job_issue(void* pJobPacket, 
	 				...
	 				const unsigned char iBus, 
	 				const unsigned char iChipToIssue,
	 				const char bAdd2msLatency);

void ASIC_job_issue_to_specified_engine(unsigned char iBus,       
	 									unsigned char iChip, 
	 									unsigned char iEngine,	
	 									void* pJobPacket, char bLoadStaticData, 
	 									char  bResetBeforStart, char bIgniteEngine, 
	 									unsigned int _LowRange, unsigned int _HighRange);
	 									
void ASIC_job_start_processing(unsigned char iBus, 
	 						   unsigned char iChip, 
	 						   unsigned char iEngine, 
	 						   char bForcedStart);
...
void ASIC_find_nonce_designated_engine(unsigned int   iNonce, 
	 								   unsigned char *iBus, 
	 								   unsigned char *iChip, 
	 								   unsigned char *iEngine);
...
int	 ASIC_tune_chip_to_frequency(unsigned char iBus, 
	 							 unsigned char iChip, 
	 							 unsigned char iEngineToUse, 
	 							 char bOnlyReturnOperatingFrequency);

int	 ASIC_are_all_engines_done(unsigned char iBus, unsigned int iChip);
void ASIC_reset_engine(unsigned char iBus, unsigned char iChip, unsigned char iEngine);
int	 ASIC_does_chip_exist(unsigned char iBus, unsigned int iChipIndex);
int	 ASIC_is_processing(void);
int	 ASIC_is_chip_processing(unsigned char iBus, unsigned char iChip); // Chip based function
int	 ASIC_is_engine_processing(unsigned char iBus, unsigned char iChip, unsigned char iEngine); // Engine based function
...
void __ASIC_WriteEngine(unsigned char iBus, 
	 					unsigned char iChip, 
	 					unsigned char iEngine, 
	 					unsigned int  iAddress, 
	 					unsigned int iData16Bit);
	 					
uint __ASIC_ReadEngine (unsigned char iBus, 
								unsigned char iChip, 
								unsigned char iEngine, 
								unsigned int iAddress);
